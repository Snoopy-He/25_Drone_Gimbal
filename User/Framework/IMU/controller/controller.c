...
/******************************* PID CONTROL *********************************/
// PID优化环节函数声明
static void f_Trapezoid_Intergral(hPID_t *pid);
static void f_Integral_Limit(hPID_t *pid);
static void f_Derivative_On_Measurement(hPID_t *pid);
static void f_Changing_Integration_Rate(hPID_t *pid);
static void f_Output_Filter(hPID_t *pid);
static void f_Derivative_Filter(hPID_t *pid);
static void f_Output_Limit(hPID_t *pid);
static void f_Proportion_Limit(hPID_t *pid);
static void f_PID_ErrorHandle(hPID_t *pid);
 ...
 * @retval         返回空
 */
float PID_Calculate(hPID_t *pid, float measure, float ref)
{
